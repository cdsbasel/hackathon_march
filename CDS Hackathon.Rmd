...
Knowing himself little about machine learning, Tom Cruise approached you to help him predict risk taking based on a short psychological assessment known as the Basel-Berlin Risk Assessment. This less than 24h assessment has been introduced by the now classic Basel-Berlin Risk Study ([**BBRS**](http://advances.sciencemag.org/content/advances/3/10/e1701381.full.pdf)) conducted almost 30 years ago by Renato Frey and colleagues. Your task is to use the BBRS's data to develop a model that predicts self-reported, yearly risk taking based on the 224 variables of this frugal measurement instrument.      

Specifcally, your goal is to outperform Tom's other friends by finding a model that has the lowest mean absolute error (MAE) in order to prove to him that you are worthy of his trust. 
...
var countDownDate = new Date("Mar 28, 2019 19:59:59").getTime();
...
1. Using `library()` load the set of packages listed in the functions section above.
...
2. With the code below, load the `bbrs` data set and change any character variables to factors.
...
1. The goal of the competition is to predict with minimal `MAE` the self-reported risk taking within a year (variable `Dy`).
...
<tr>
  <td>Regression</td> 
  <td>Yearly risk taking (`Dy`)</td>
  <td>MAE</td>
  <td>[**Submit candidate**](https://www.dropbox.com/request/QapEfuzEL4JM9bR62hXZ)</td>
</tr>
</table>
</div>

5. Use any weapon in your arsenal (or `caret`'s arsenal). Feel free to try different models, use different tuning parameter settings or preprocessing methods, make use of all or some variables. Whatever may lead to the highest prediction `MAE`. For helo take a look at the full [**course materials**](https://therbootcamp.github.io/appliedML_2019Jan/). 
